...
`build-bhyve-image.sh` remains source-based. It drives the FreeBSD release `vm-image` target with raw/UFS output and `vmimage.conf`. The configuration adds Python, ipykernel, cloud-init, the `freebsd` account, and the restricted SSH policy before the image is unmounted. It pre-creates the target `/usr/local/lib` directory before FreeBSD's initial linker-cache setup, then refreshes the target image's linker cache after each package-installation phase before executing the target Python interpreter.
